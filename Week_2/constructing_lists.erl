...
- export([double/1, even/1, median/1, sort/1, mode/1, frequency/2]).
...
% frequency by pattern matching
frequencyp([X], N) ->[{X,N}];
frequencyp([X,X|Xs],N) -> frequencyp([X|Xs], N+1);
frequencyp([X,Y|Xs],N) -> [{X,N}| frequencyp([Y|Xs], 1)].
